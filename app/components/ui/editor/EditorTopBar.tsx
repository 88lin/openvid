...
export function EditorTopBar({
    ...
    const displayName = profile?.first_name || profile?.full_name || meta.full_name || meta.name || user?.email?.split("@")[0] || "Usuario";
        ...
        <div className="h-13 border-b border-white/10 flex items-center justify-between px-4 shrink-0 relative">
            ...
            <div className="flex items-center gap-3 ml-auto">
                <div className="flex items-center gap-2 border-r border-white/10 pr-3">
                    <TooltipAction label={canUndo ? "Deshacer (Ctrl+Z)" : "No hay acciones para deshacer"}>
                        <button
                            onClick={onUndo}
                            disabled={!canUndo}
                    ...
                    <TooltipAction label={canRedo ? "Rehacer (Ctrl+Y)" : "No hay acciones para rehacer"}>
                        <button
                            onClick={onRedo}
                            disabled={!canRedo}
                        ...
                        <DropdownMenu.Trigger asChild>
                            ...
                            >
                                <div className="hidden sm:flex flex-col items-end leading-none">
                                    <span className="text-[11px] font-medium text-white max-w-[100px] truncate">
                                        {displayName}
                                    </span>
