  ...
  return new ShaderMaterial({
    ...
    fragmentShader: `
      ...
      vec4 sampleScreen(vec2 uv, float lod) {
        ...
        vec4 background = sampleLayer(screenMap, (screenTransform * vec3(backgroundUv, 1.0)).xy, lod);
        if (isVideo > 0.5) background = sRGBTransferEOTF(background);
        vec2 revealUv = uv;
